...
from engines import ev_engine, probability_engine as research_probability_engine
...
def _section_ev_probability_lab():
    ev = ev_engine.trade_expected_value(0.56, 0.035, 0.024, cost=0.004)
    bayes = research_probability_engine.bayes_update(0.52, 0.68, 0.38)
    drawdown = research_probability_engine.probability_of_drawdown(0.08, 0.22, 0.10, horizon_scale=0.25)
    return f"""
    <section id="ev-probability-lab">
      <h2>EV & Probability Lab</h2>
      <p class="workspace-question">Is this idea positive expected value after probability, costs, uncertainty, and drawdown risk?</p>
      <div class="hero-grid">
        {_mini_card('Trade EV After Costs', _fmt(ev.get('result'), precision=4), '#2ee59d' if ev.get('result', 0) > 0 else '#ff5c75')}
        {_mini_card('Bayesian Posterior', _pct(bayes.get('result'), multiply=True), '#5db7ff')}
        {_mini_card('Drawdown Probability', _pct(drawdown.get('result'), multiply=True), '#ffd166')}
      </div>
      <div class="two-col">
        <div class="panel hover-card"><h3>Expected Value Calculator</h3><p><b>Formula:</b> {html.escape(ev.get('formula_used'))}</p><p>{html.escape(ev.get('plain_english_explanation'))}</p><p><b>Weakness:</b> {html.escape(ev.get('weaknesses')[0])}</p></div>
        <div class="panel hover-card"><h3>Bayes Updater</h3><p><b>Formula:</b> {html.escape(bayes.get('formula_used'))}</p><p>{html.escape(bayes.get('plain_english_explanation'))}</p><p><b>What changes it:</b> {html.escape(bayes.get('what_would_change_the_result')[0])}</p></div>
      </div>
    </section>"""
...
def generate_dashboard(
    ...
    sec_ev_probability = _section_ev_probability_lab()
    ...
    pages = {
        ...
        "ev-probability": sec_ev_probability,
    ...
    nav_groups = [
        ...
        ("🧪 Research Labs", [("Research Labs", "research-labs"), ("EV & Probability", "ev-probability"), ("Portfolio Lab", "portfolio-lab"), ("Monte Carlo Lab", "monte-carlo-lab"), ("Volatility Lab", "volatility-lab"), ("Factor Research", "factor-research"), ("Options Lab", "options-lab"), ("Forecasting Lab", "forecasting-lab"), ("Optimization Lab", "optimization-lab"), ("Scenario Lab", "scenario-lab")]),
...
<head>
  ...
  <style>
    *{{box-sizing:border-box;}}
    :root{{--bg:#06080d;--surface:rgba(15,22,34,.88);--surface2:rgba(22,32,49,.94);--border:rgba(121,192,255,.16);--text:#e7eef8;--muted:#92a4b8;--accent:#2ee59d;--blue:#5db7ff;--yellow:#ffd166;--red:#ff5c75;}}
    body{{margin:0;font-family:Inter,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:radial-gradient(circle at 10% 0%,rgba(49,116,255,.18),transparent 30%),radial-gradient(circle at 90% 12%,rgba(46,229,157,.12),transparent 28%),linear-gradient(135deg,#06080d 0%,#0a101b 50%,#05070b 100%);color:var(--text);}}
    ...
    .metric-card,.panel{{background:linear-gradient(180deg,var(--surface2),var(--surface));border:1px solid var(--border);border-radius:18px;padding:18px;box-shadow:0 18px 55px rgba(0,0,0,.34);}}
