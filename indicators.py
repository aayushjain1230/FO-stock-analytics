...
def calculate_metrics(df, benchmark_df):
    ...
    # ------------------------------------------------------------
    # Align CLOSE series only (critical fix)
    # ------------------------------------------------------------
    df_close, bench_close = df["Close"].align(
        benchmark_df["Close"], join="inner"
    )
    df = df.loc[df_close.index]
    ...
    # ------------------------------------------------------------
    df["Vol_20_Avg"] = pd.to_numeric(
        ta.sma(df["Volume"], length=20), errors="coerce"
    )
    ...
    df["RS_SMA50"] = pd.to_numeric(
        ta.sma(df["RS_Line"], length=50), errors="coerce"
    )
    df["RS_SMA20"] = pd.to_numeric(
        ta.sma(df["RS_Line"], length=20), errors="coerce"
    )
    ...
    df["ATR"] = pd.to_numeric(
        ta.atr(df["High"], df["Low"], df_close, length=14),
        errors="coerce",
    )
    ...
    df["RSI"] = pd.to_numeric(
        ta.rsi(df_close, length=14), errors="coerce"
    )
    ...
    df_weekly["RSI_W"] = pd.to_numeric(
        ta.rsi(df_weekly["Close"], length=14),
        errors="coerce",
    )
    df["RSI_Weekly"] = df_weekly["RSI_W"].reindex(
        df.index, method="ffill"
    )

    # Monthly (use stable 'M', not 'ME')
    df_monthly = df.resample("M").apply(ohlc_dict)
    df_monthly["RSI_M"] = pd.to_numeric(
        ta.rsi(df_monthly["Close"], length=14),
        errors="coerce",
    )
    df["RSI_Monthly"] = df_monthly["RSI_M"].reindex(
        df.index, method="ffill"
    )
    ...
    df["RS_Breakout"] = (
        (df["MRS"] > 0) & (df["MRS"].shift(1) <= 0)
    ).fillna(False)

    # ------------------------------------------------------------
    # 8. Market Regime Label (metadata only)
    # ------------------------------------------------------------
    df.attrs["market_regime"] = get_market_regime_label(
        benchmark_df.loc[df.index]
    )
...
def get_market_regime_label(spy_df):
    ...
    try:
        ...
        # Normalize ALL outputs to Series (critical fix)
        if isinstance(sma_raw, pd.Series):
            ...
            sma_series = pd.Series(
                [sma_raw] * len(close), index=close.index
            )
        else:
            sma_series = pd.Series(
                sma_raw, index=close.index
            ).astype(float)
...
def calculate_market_leader_score(row):
    ...
    # ------------------------------------------------------------
    if (
        pd.notna(row.get("SMA50"))
        and pd.notna(row.get("SMA200"))
    ...
    if (
        pd.notna(row.get("RS_Line"))
        and pd.notna(row.get("RS_SMA20"))
        and row["RS_Line"] > row["RS_SMA20"]
    ):
        score += 10
